...
async function calculateScoresForAllUsers() {
    ...
    for (const guess of guesses.rows) {
        ...
        // Calcul du score pour la date et l'heure séparément
        ...
        const guessedTime = guessedBirthdate.toTimeString().split(" ")[0].substring(0, 5);
        const actualTime = actualBirthdate.toTimeString().split(" ")[0].substring(0, 5);
        if (guessedTime === actualTime) {
            score_time = 2;
        }
